...
public class Randomizer
{
    ...
    public static String randomizeValue( String value )
    {
        ...
        if ( tempValue != null )
        {
            ...
            log.debug( "After replacing placeholders with random upper case letters: " + tempValue );

            log.debug( "Replacing placeholders with random lower case letters" );
            tempValue = replaceAllOccurrences( tempValue,
                                               REPLACE_WITH_LOWER_CASE_LETTERS );
            log.debug( "After replacing placeholders with random lower case letters: " + tempValue );

            log.debug( "Replacing placeholders with random digits" );
            tempValue = replaceAllOccurrences( tempValue,
                                               REPLACE_WITH_DIGITS );
            log.debug( "After replacing placeholders with random digits: " + tempValue );
        }
                                     ...
                                     int length )
    {
        ...
        log.debug( "Generating random unit with size " + length );
                                                 ...
                                                 int replaceSeed )
    {
        ...
        log.debug( "Pattern for matching is build: " + pattern.toString( ) );
        log.debug( "Matching the provided value with built pattern: " + value );
        ...
        if ( matcher.find( ) )
        {
            ...
            for ( int groupIndex = 0; groupIndex <= matcher.groupCount( ); groupIndex++ )
            {
                ...
                String replacement = randomize( arr,
                                                length );
                log.debug( "Replacing matched item with: " + replacement );
        ...
        log.debug( "Exiting replaceAllOcurances() method" );
        return value;
    }
}
